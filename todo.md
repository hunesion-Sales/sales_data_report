...
#### 14-1. 제품별 통계 및 차트 구현
57. [x] **통계 카드 및 차트 추가**:
    - `ProductCharts.tsx`: 기간별 매출 추이, 제품별 점유율, 매출/이익 구조 시각화
    - `ProductReportPage.tsx`: 상단 KPI 카드(매출, 이익, 이익률) 추가
    - 데이터 정합성: `cloud_total` 중복 집계 방지 로직 적용

### Phase 15: 차트 레이아웃 개선 및 보기 모드 (2026-02-18)

#### 15-1. 보기 모드 토글 및 레이아웃 리팩토링
58. [x] **매출/이익 보기 토글**:
    - 부문별/제품별 보고서 상단에 '매출액 보기' / '매출이익 보기' 버튼 추가
    - 토글 시 모든 차트/통계가 해당 지표로 즉시 전환
59. [x] **차트 레이아웃 최적화**:
    - 기간별 추이 차트: 12개월 데이터 가독성을 위해 전체 너비(Full Width)로 확장
    - 순위 차트: 복잡한 비교 차트 대신 직관적인 순위(Ranking) 차트로 변경
