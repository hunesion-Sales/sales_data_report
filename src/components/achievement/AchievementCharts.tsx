...
export default function AchievementCharts({ achievements }: AchievementChartsProps) {
  // Grouped Bar Chart: 부문별 목표 vs 실적
  const barData = useMemo(() => {
    return achievements.map(a => ({
      name: a.divisionName,
      목표: a.target.salesTarget,
      실적: a.actualSales,
    }));
  }, [achievements]);
  ...
  const radialData = useMemo(() => {
    return achievements.map((a, idx) => ({
      name: a.divisionName,
      value: Math.round(a.salesAchievementRate * 10) / 10,
        ...
        <div className="h-80 min-h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={barData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Bar dataKey="목표" fill="#94a3b8" radius={[4, 4, 0, 0]} barSize={24} />
              <Bar dataKey="실적" fill="#6366f1" radius={[4, 4, 0, 0]} barSize={24} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
