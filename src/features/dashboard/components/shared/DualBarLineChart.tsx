...
export interface DualBarLineChartDataItem {
  ...
  backlog: number;         // 수주잔액 (바 3: 베이지)
  achievementRate: number; // 달성율 (라인 1: 남색)
...
interface DualBarLineChartProps {
  ...
  title?: string;
  height?: number;
  /** 라인 차트 표시 개수: 2(달성율+성장율) 또는 1(달성율만) */
  lineCount?: 1 | 2;
  /** 바 데이터키 재정의 (부문별 차트용) */
  barOverrides?: {
    bar1Key?: string;
...
const BAR_COLORS = {
  prevYear: '#cbd5e1',    // slate-300 (회색)
  current: '#1e3a5f',     // 남색
  backlog: '#fde68a',     // amber-200 (베이지)
};

const LINE_COLORS = {
  achievement: '#1e3a5f', // 남색
    ...
    <ChartWrapper title={title} height={height} loading={loading} hasData={data.length > 0}>
      <ComposedChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
        ...
        <Bar yAxisId="left" dataKey={bar1Key} name={bar1Name} fill={bar1Color} barSize={20} radius={[2, 2, 0, 0]}>
          <LabelList dataKey={bar1Key} position="top" formatter={(v: any) => formatMillionWonChart(v)} fontSize={9} fill="#94a3b8" />
        </Bar>

        {/* 바 2: 당년 실적 */}
        <Bar yAxisId="left" dataKey={bar2Key} name={bar2Name} fill={bar2Color} barSize={20} radius={[2, 2, 0, 0]}>
          <LabelList dataKey={bar2Key} position="top" formatter={(v: any) => formatMillionWonChart(v)} fontSize={9} fill="#1e3a5f" />
        </Bar>

        {/* 바 3: 수주잔액 */}
        <Bar yAxisId="left" dataKey={bar3Key} name={bar3Name} fill={bar3Color} barSize={20} radius={[2, 2, 0, 0]}>
          <LabelList dataKey={bar3Key} position="top" formatter={(v: any) => formatMillionWonChart(v)} fontSize={9} fill="#d97706" />
        </Bar>
        ...
        <Line
          ...
          name="달성율"
          stroke={LINE_COLORS.achievement}
          strokeWidth={2}
          dot={{ r: 3, fill: LINE_COLORS.achievement }}
          connectNulls
        />
        ...
        {lineCount === 2 && (
          <Line
            ...
            dataKey="growthRate"
            name="성장율"
            stroke={LINE_COLORS.growth}
            strokeWidth={2}
            strokeDasharray="5 5"
            dot={{ r: 3, fill: LINE_COLORS.growth }}
            connectNulls
          />
        )}
      </ComposedChart>
